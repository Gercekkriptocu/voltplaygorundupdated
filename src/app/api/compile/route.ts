...
// OpenZeppelin v5.0.0 contracts (embedded for imports)
  ...
  try {
    ...
    if (!sourceCode || !contractName) {
      return NextResponse.json(
        { success: false, error: 'Missing sourceCode or contractName' },
        { status: 400 }
      );
    }
    ...
    if (!contract) {
      return NextResponse.json({
        success: false,
        error: `Contract "${contractName}" not found. Available contracts: ${Object.keys(
          output.contracts['main.sol'] || {}
        ).join(', ')}`,
      });
    }
    ...
    if (!bytecode || bytecode.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'Compiled successfully but no bytecode generated',
      });
    }
